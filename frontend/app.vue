<template>
  <div style="font-family: system-ui, sans-serif; max-width: 1000px; margin: 40px auto; padding: 20px; background: #0f172a; color: #f8fafc; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.3);">
    <header style="border-bottom: 1px solid #334155; padding-bottom: 16px; margin-bottom: 24px;">
      <h1 style="margin: 0; font-size: 24px; color: #38bdf8;">🧠 Pi-JEV-DSK Enterprise AGI Control Center</h1>
      <p style="margin: 4px 0 0; color: #94a3b8; font-size: 14px;">Powered by @deepseek-ai/cordis, @earendil-works/pi-coding-agent, and @typesafe-ai/jev.</p>
    </header>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
      ...
      <div style="background: #1e293b; padding: 20px; border-radius: 8px; border: 1px solid #334155;">
        ...
        <textarea v-model="goal" rows="4" style="width: 100%; padding: 10px; background: #0f172a; border: 1px solid #475569; color: #fff; border-radius: 6px; resize: vertical; box-sizing: border-box;" placeholder="e.g., Analyze and optimize distributed computing workloads..."></textarea>
        ...
        <button @click="runAGI" :disabled="loading" style="margin-top: 12px; background: #0284c7; color: white; border: none; padding: 10px 20px; font-weight: bold; border-radius: 6px; cursor: pointer; width: 100%;">
          {{ loading ? 'Executing Enterprise DSK Loop...' : 'Execute AGI Task' }}
        </button>
...
const runAGI = async () => {
  loading.value = true
  result.value = null
  trace.value = ['[UI] Dispatching goal to Enterprise Cordis AGI container...']
